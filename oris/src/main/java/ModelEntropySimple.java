...
public class ModelEntropySimple {
	...
	public static void build(PetriNet net, Marking marking) {
		...
		Transition T_serv_start = net.addTransition("T_serv_start");
		...
		net.addPrecondition(S_fin, T_serv_start);
		net.addPrecondition(S_serv, T_serv);
		net.addPostcondition(T_serv_start, S_serv);
		...
		net.addPrecondition(S_lb, T_serv_start);
		net.addPostcondition(T_impact, S_fin);
		net.addPostcondition(T_arrival, S_lb);

		// Generating Properties
		marking.setTokens(S_check, 0);
		marking.setTokens(S_exec_tasks, 0);
		marking.setTokens(S_fin, r);
		marking.setTokens(S_lb, 0);
		marking.setTokens(S_serv, 0);
		T_arrival.addFeature(new EnablingFunction(String.format("S_lb<%d", k)));
